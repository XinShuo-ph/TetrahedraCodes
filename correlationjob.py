...
for centerP in samplePcen:
    if cnt%1000 ==0:
        np.save(corrdir+"cnt_%d.npy"%(cnt),np.array([]))
    cnt+=1
...
# replaced by vectorized version above
#    for spherePunit in samplePsph:
#        sphereP = centerP + sampleradius*spherePunit
#        deltasph = treefield[mytree.query(sphereP)[1]]
#        samplexi.append(deltacen*deltasph)

os.system("rm "+corrdir+"cnt*.npy\n")
np.save(corrdir+"samplexi.npy",samplexi)
np.save(corrdir+"sampleradius.npy",sampleradius)
np.save(corrdir+"meanxi.npy",np.mean(samplexi))