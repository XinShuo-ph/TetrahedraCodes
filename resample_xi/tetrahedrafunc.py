import numpy as np
#20230220: added interpolated pts

def get_tet_interpolated_particles(Ndim,p3d, split=10, rand = True, edge = False, get_density = False):
    """ A fast function to create particles inside tetrahedra 
        with the same linear map that made the tet. """
    if (not rand) and (not edge):
        mysplitn = split
        split = int( (mysplitn*(mysplitn+1)*(mysplitn+2))/6 )
    ...
    if get_density:
        newvols = np.zeros((Np*Nnppp))
    if rand:
        ro = np.random.random(size=(Np*Nnppp,3)) # random offsets
        # fold the points back to the tetrahedra: http://vcg.isti.cnr.it/jgt/tetra.htm
        case1 = np.where(ro[:,0]+ro[:,1]<=1)[0]
        case2 = np.where(ro[:,0]+ro[:,1]>1)[0]
        ro[case2,0] = 1-ro[case2,0]
        ro[case2,1] = 1-ro[case2,1]
        case1 = np.where(ro[:,0]+ro[:,1]+ro[:,2]<=1)[0]
        case2 = np.where(ro[:,1]+ro[:,2]>1)[0]
        case3 = np.intersect1d(np.where(ro[:,1]+ro[:,2]<=1)[0], np.where(ro[:,0]+ro[:,1]+ro[:,2]>1)[0])
        ro[case2,1],ro[case2,2] = (1-ro[case2,2], 1-ro[case2,0]-ro[case2,1])
        ro[case3,0],ro[case3,2] = (1-ro[case3,1]-ro[case3,2], ro[case3,0]+ro[case3,1]+ro[case3,2]-1)
    else:
        x=np.linspace(1e-3,1-1e-2,mysplitn)
        xg = np.meshgrid(x,x,x, indexing='ij')
        xl = np.ravel(xg[0])
        yl = np.ravel(xg[1])
        zl = np.ravel(xg[2])
        ro=np.vstack([xl,yl,zl]).transpose()
        ro = ro[np.where(ro[:,0]+ro[:,1]+ro[:,2]<1)[0]] # select only one tetrahedron
    for m in range(Ntetpp):   # 6 tets
        ...
        a =  ( p3d[off[0][0]:(Ndim+off[0][0]),off[0][1]:(Ndim+off[0][1]),off[0][2]:(Ndim+off[0][2]), :] \
            - orig).reshape((Np,3))
        if get_density:
            vols = (-np.sum(a*np.cross(b,c),axis=1)/6.0)
        for s in range(split):
            if get_density:
                newvols[m*split+s::Nnppp] =vols
            for i in range(3):
                # correcting the index
                ## there's a simple correction to fold the points back to the tetrahedra: http://vcg.isti.cnr.it/jgt/tetra.htm
                if rand:
                    newp[m*split+s::Nnppp,i] = orig.reshape(Np,3)[:,i] + \
                    ((ro[m*split+s::Nnppp,0]*a[:,i]+ro[m*split+s::Nnppp,1]*b[:,i]+ro[m*split+s::Nnppp,2]*c[:,i]))
                else:
                    #print("test")
                    newp[m*split+s::Nnppp,i] = orig.reshape(Np,3)[:,i] + \
                    ((ro[s,0]*a[:,i]+ro[s,1]*b[:,i]+ro[s,2]*c[:,i]))
    if get_density:
        return newp,newvols
    else:
        return newp
